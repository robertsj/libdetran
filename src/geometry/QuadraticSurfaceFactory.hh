...
struct QuadraticSurfaceFactory
{
  ...
  {
    return std::make_shared<QS>(0, 0, 0,   0, 0, 0,   1, 0, 0,  -x);
  }

  static SP_surface CreatePlaneY(c_dbl y)
  {
    return std::make_shared<QS>(0, 0, 0,   0, 0, 0,   0, 1, 0,  -y);
  }

  static SP_surface CreatePlaneZ(c_dbl z)
  {
    return std::make_shared<QS>(0, 0, 0,   0, 0, 0,   0, 0, 1,  -z);
  }

  /// a*x + b*y + c*z = d
  static SP_surface CreatePlane(c_dbl a, c_dbl b, c_dbl c, c_dbl d)
  {
    return std::make_shared<QS>(0, 0, 0,   0, 0, 0,   a, b, c,  -d);
  }

  /// (y-y0)^2 + (z-z0)^2 = r^2
  static SP_surface CreateCylinderX(c_dbl y0, c_dbl z0, c_dbl r)
  {
    return std::make_shared<QS>(0, 1, 1,   0, 0, 0,   0, -2*y0, -2*z0,   y0*y0+z0*z0-r*r);
  }

  /// (x-x0)^2 + (z-z0)^2 = r^2
  static SP_surface CreateCylinderY(c_dbl x0, c_dbl z0, c_dbl r)
  {
    return std::make_shared<QS>(1, 0, 1,   0, 0, 0,   -2*x0, 0, -2*z0,   x0*x0+z0*z0-r*r);
  }

  /// (x-x0)^2 + (y-y0)^2 = r^2
  static SP_surface CreateCylinderZ(c_dbl x0, c_dbl y0, c_dbl r)
  {
    return std::make_shared<QS>(1, 1, 0,  0, 0, 0,  -2*x0, -2*y0, 0,  x0*x0+y0*y0-r*r);
  }

  /// (x-x0)^2 + (y-y0)^2 + (z-z0)^2 = r^2
  static SP_surface CreateSphere(c_dbl x0, c_dbl y0, c_dbl z0, c_dbl r)
  {
    return std::make_shared<QS>(1,1,1, 0,0,0, -2*x0,-2*y0,-2*z0, x0*x0+y0*y0+z0*z0-r*r);
  }

...
int test_Equation_SC_1D(int argc, char *argv[])
{
  ...
  Equation_SC_1D::angular_flux_type psi(mesh->number_cells(), 0.0);

  // Cell sweep source [n/cm^2-s-ster]
  Equation_SC_1D::moments_type source(mesh->number_cells(), 1.0);

  // Create incident and outgoing face fluxes
  Equation_SC_1D::face_flux_type psi_in  = 0.0;
  Equation_SC_1D::face_flux_type psi_out = 0.0;
